...
\usepackage{graphicx, amsmath, amsthm, amssymb, enumitem, lipsum, algorithm, caption, fancyhdr}
...
\section{Intro / Basic Definitions}
...
\end{center}
Here, a simple breadth-first-search on $G$ will give the optimal solution in $O(|V| + |E|)$ time. Others, however, are not so easy:
\begin{center}
    Given a map $M$, what is the minimum number of colors needed\\ to color $M$ so that no two neighboring regions share the same color? 
\end{center}
...
\begin{theorem}{3.1}{}
    ...
    \begin{proof}
        ...
        \[
        c(\mathcal{H}) \leq c(\mathcal{P}) = 2 \cdot c(T) \leq 2 \cdot \text{OPT},
        \]
...
\\
A key result in graph theory is that a connected graph has an Eulerian tour if and only if every vertex has even degree. So, we only need to concern ourselves with somehow ``remedying'' the odd-degree vertices in the MST to obtain a an Eulerian graph. The way we remedy these vertices is by using a \textit{perfect matching}. A perfect matching is quite similar in notion to a bipartite matching, which the reader should be familiar with, but for the sake clarity, let's give the definition in full generality.

\begin{definition}{(Perfect Matching)}{}
    Given an undirected graph $G = (V,E)$ and subset $U \subseteq V$, a \textit{perfect matching on $U$} is a subset $M \subseteq E$ such that every vertex in $U$ is incident to exactly one edge from $M$.
\end{definition}
...
\\
Regardless, we now have a graph that we can perform an Eulerian tour on! The approach we have just described is \href{https://en.wikipedia.org/wiki/Christofides_algorithm}{\underline{due to  Christofides}}. Using his idea, it's possible to ensure a 3/2 approximation guarantee for Metric TSP.
...
\begin{lemma}{3.2}{}
    ...
    \begin{proof}
        Let's quickly recall that the \textit{handshake lemma} tells us the sum of degrees of all vertices in an undirected graph must be even. From this, our claim follows immediately. If there were an odd number of odd-degree vertices, then the total sum of vertex degrees would be odd, which is impossible.
...
\begin{lemma}{3.3}{}
    Let $G = (V,E)$ be a graph given as an instance of the Metric TSP problem. Let $U \subseteq V$ such that $|U|$ is even, and let $M$ be a minimum cost perfect matching on $U$. Then, $c(M) \leq \text{OPT}/2$.\\
    \\
    ...
    \[
        c(M) \leq \min \{c(M_1),c(M_2)\} \leq c(\tau')/2 \leq \text{OPT}/2.\quad \Box
        \]
\end{lemma}
...
\begin{theorem}{4.1}{}
    ...
    \begin{proof}
        ...
        \begin{align*}
            \mathbb{E}[Y] &= \mathbb{E} \left[ \sum_{i=1}^m Y_i \right]\\
...
\vspace{-2ex}
\begin{theorem}{4.3}
    \textsc{Approx-Min-Weight-VC} is a 2-approximation algorithm for the Min-Weight Vertex Cover problem.
