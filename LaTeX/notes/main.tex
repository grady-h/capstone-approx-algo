...
\noindent Below is a depiction of the algorithm in action. Figure (a) is the original complete graph $G$, with Figure (b) showing a minimum spanning tree $T$ of $G$. Figure (c) outlines the preorder traversal $\mathcal{P}$ of $T$. Lastly, Figure (d) is the resulting Hamiltonian cycle $H$ constructed from the $T$, where the backtracking steps in $\mathcal{P}$ have been ``short-cut'' for more direct paths.  
...
\begin{lemma}{5.1}{}
    Fix $\varepsilon > 0$. Let $S$ be the solution returned by \textsc{Knapsack-FPTAS} run with parameter $\varepsilon$, and let $O$ be the true optimal solution. Then,
    % \[
...
\begin{theorem}{5.2}{}
    ...
    \begin{proof}
        ...
        Given $\varepsilon > 0$, we'll run our algorithm with a new parameter $\delta = \frac{\varepsilon}{1 + \varepsilon}$. We can use the bound from Lemma 5.1 to see that
        \begin{align*}
